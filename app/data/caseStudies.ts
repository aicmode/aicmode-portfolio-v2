...
export const caseStudies: readonly CaseStudy[] = [
  ...
  },
  {
    ...
    features: [
      ...
      'OpenAI APIによる議事録の生成',
      'Google Docs APIでのドキュメント作成と書き込み',
      'Google Drive APIでの指定フォルダへの移動',
      'ファイル指定なしで最新録音を自動選択',
      '各ステップの進行状況をログ表示',
    ],
    stack: ['Python', 'OpenAI Whisper API', 'OpenAI API', 'Google Docs API', 'Google Drive API'],
    accent: '#d4af37',
    detail: {
      goal: '会議終了から議事録の共有までを、人の作業なしで通せる状態にすること。',
      proposedSolution: [
        '文字起こしと要約を分け、失敗した段だけを再実行できるようにする',
        '出力先を既存のGoogleドキュメントにして、共有と編集の運用を変えない',
        '保存フォルダをDrive APIで指定し、探す手間をなくす',
        '録音ファイルの選択を自動化し、実行時の入力を不要にする',
      ],
      mvpScope: [
        '録音ファイル（mp3 / mp4 / m4a / wav / webm）の文字起こし',
        '議事録の生成',
        'Googleドキュメントの作成と書き込み',
        '指定フォルダへの移動（任意）',
        '初回のOAuth認証と以降のトークン再利用',
      ],
      phase2: [
        '25MB超のファイルの自動分割',
        '決定事項・宿題・期限の構造化抽出',
      ...
      architecture: [
        '`main.py` が 文字起こし → 要約 → ドキュメント作成 の順で制御',
        '各段を `transcribe.py` / `summarize.py` / `google_docs.py` に分離',
        '設定値は `config.py` に集約',
        '認証は初回のみブラウザで実行し、`token.json` を再利用',
      ],
      security: [
        'APIキーは `.env`、Google認証は `credentials.json` / `token.json` で管理し、いずれもGit管理外',
        'Google APIのスコープはDocs / Driveの必要範囲のみ',
        '録音・文字起こしデータはローカルに置き、共有先はDriveのフォルダで限定',
        '会議内容は機微情報のため、送信先サービスの規約確認を前提とする',
      ],
      expectedImpact: [
        '議事録作成にかかる作業の削減を目指す',
        '録音を聞き直す時間の短縮を想定',
        '議事録の書式と粒度を揃えることで、後から探しやすくする設計',
      ],
    },
    githubUrl: 'https://github.com/aicmode/google-meet-minutes-generator',
    order: 5,
    year: 2026,
  },
  {
